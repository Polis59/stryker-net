...
public class SingleMicrosoftTestPlatformRunner : IDisposable
{
        ...
        Func<TestNode, bool>? testUidFilter)
    {
        ...
        finally
        {
            _logger.LogInformation(
                "{RunnerId}: BATCHTIME mutants={Mutants} tests={Tests} ms={ElapsedMs}",
                RunnerId, mutants.Count, GetAssessingTestCount(mutants), batchStopwatch.ElapsedMilliseconds);
    ...
    internal TimeSpan? CalculateAssemblyTimeout(List<TestNode> discoveredTests, ITimeoutValueCalculator timeoutCalc, string assembly, bool serialActivation = false)
    {
        ...
        // Truth mode: the MTP protocol reports no per-test timing, so the estimate smears the
        // initial run's duration evenly across tests, and map-activated batches additionally run
        // serially with per-test switching costs the parallel initial run never paid. A tight
        ...
        Func<TestNodeUpdate, bool>? bailPredicate = null)
    {
        try
        {
            ...
            // of the session has a verdict. A killing test resolves its assigned mutant; when the
            // set of unresolved mutants empties, the remaining mapped tests prove nothing. A
            // session containing a true survivor never bails — the survivor needs its whole
            // assessing set, exactly as under the stock VSTest runner. In-flight tests cancelled
            // by the bail are attributed as failures of already-resolved mutants only, so no
            // verdict rests on a cancelled test.
            if (bailPredicate is null && !useCollectibleIsolation && wholeSessionActivation && mutants is { Count: 1 })
            {
                ...
                // bail, without needing the (inactive) assignment map.
            ...
            if (bailPredicate is null && mapAssignments is { Count: > 0 } && mutants is { Count: > 0 })
            {
                ...
                bailPredicate = update =>
                {
                    ...
                    if (!mapAssignments.TryGetValue(update.Node.Uid, out var ownerId))
                    {
                        return false;
                    }
